function GIFGroover() {
  ...
  var interlacedBufSize,
    ...
    bgColourIndex,
    globalColourTable;
  const bitValues = new Uint32Array([
    1,
    2,
    4,
    8,
  ...
  function processFrame(frame) {
    ...
    const ct = frame.localColourTableFlag
      ? frame.localColourTable
      : globalColourTable;
    lastFrame = lastFrame ? lastFrame : frame;
    useT =
      lastFrame.disposalMethod === 2 || lastFrame.disposalMethod === 3
        ? true
        : false;
    if (!useT) {
  ...
  function getFrameAtTime(timeMs) {
    ...
    while (
      frame < frames.length &&
      timeMs > frames[frame].time + frames[frame].delay
    ) {
      frame += 1;
    }
    return frame;
    ...
    if (playSpeed === 0) {
      ...
      if (nextFrameTime !== undefined && nextFrame === null) {
        ...
        if (behind < -frameTime / 2) {
          framesSkipped = true;
          nextFrameTime =
            ((nextFrameTime + behind / playSpeed) % duration) + duration; // normalize to positive
  ...
  const gif = {
    ...
    getFrame(index) {
      ...
      return frames[
        index < 0 ? 0 : index >= frames.length ? frames.length - 1 : index
      ].image;
    },
