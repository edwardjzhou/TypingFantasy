...
export default class UI {
  static styleTooltip() {
    ...
    Object.assign(
      document.querySelector(`#cheat-tooltiptext`).style,
      tooltiptextStyle
    );
  ...
  static createScoreTable(scores) {
    const { noInconsistency: noInconsistency, sorted } = this.ensureSorted(
      scores
    );
    ...
    for (let [index, [score, name]] of Object.entries(scores)) {
      table.appendChild(textNode(`${++index}. ${name} scored ${score}`));
  ...
  static attachTableToCanvasRight(table, yetToAttach = true) {
    ...
    if (yetToAttach)
      window.addEventListener("resize", () =>
        this.attachTableToCanvasRight(table, false)
      );
  }
  ...
  static addCheats() {
    ...
    email.addEventListener(
      `mouseenter`,
      () => {
        if (!window.game.onSplash) window.game.rate = 0.66 * window.game.rate;
      },
      false
    );
  }
