...
class Game {
  ...
  #dynamicallyLoadLanguageModule() {
    ...
    if (this.gameMode === `english`) {
      import(
        /* webpackChunkName: "./dictionaryEnglish", webpackPrefetch: true */ "./dictionaryEnglish"
      )
        .then(({ englishDictionary }) => {
      ...
      import(
        /* webpackChunkName: "./dictionaryChinese", webpackPrefetch: true */ "./dictionaryChinese"
      )
  ...
  animateSplash(timeElapsed = 0) {
    ...
    this.ctx.drawImage(
      ...
      this.canvas.width,
      this.canvas.height
    ); //add credits of where i took image from
    ...
    this.ctx.strokeText(
      "TYPING FANTASY!",
      this.canvas.width * 0.1 - 2,
      100 - 3
    );
    this.ctx.fillText("TYPING FANTASY!", this.canvas.width * 0.1, 100);
    ...
    this.ctx.strokeStyle = "white";
    this.ctx.strokeText(
      "Choose a TYPING LANGUAGE and hit ENTER!",
      this.canvas.width * 0.05 - 2,
      this.canvas.height * 0.5 - 3
    );
    this.ctx.fillText(
      "Choose a TYPING LANGUAGE and hit ENTER!",
    ...
    if (this.gameMode === `english`)
      this.ctx.drawImage(this.cursor, 0, 0, 32, 32, 25, 320, 50, 50);
    else if (this.gameMode === `chinese`)
      this.ctx.drawImage(this.cursor, 0, 0, 32, 32, 25, 380, 50, 50);
  ...
  drawAttackArc() {
    // this remains static after an attack until a new attack. could let chrono class take care of this.
    // THIS IS WHERE THE ARC IS DRAWN)
    if (this.player.circleCenterY) {
      // first line of two
      this.ctx.beginPath();
      this.ctx.arc(
        this.player.circleCenterX,
        this.player.circleCenterY,
        this.player.diameter / 2,
        this.player.angleStartClockwise,
        this.player.angleEndClockwise,
        false
      );
  ...
  drawEnemies() {
    ...
    for (let i = 0; i < this.enemies.length; i++) {
      ...
      if (
        possibilities.includes(this.enemies[i].word) &&
        this.enemies[i].alive === true
      )
        this.enemies[i].animateReticle();
    }
  }
  ...
  drawWPM() {
    if (this.isPaused && !this.timeOfPauseStart)
      this.timeOfPauseStart = Date.now();
    if (!this.isPaused && this.timeOfPauseStart) {
  ...
  gameLoop() {
    //responsible for spawning initial actors, creating new actors, and calling animate-- will eventually stick all game logic in here including calls to "move/act" rather than letting animate take care of that implicitly
  ...
  _addListeners() {
    ...
    if (this.gameMode === `english`) {
      this.keydownHandler = (e) => {
        ...
        if (key >= 65 && key <= 90)
          this.word.push(String.fromCharCode(key).toLowerCase());
      ...
      const attachChineseInputToCanvasBottomLeft = (yetToAttach = true) => {
        this.chineseInput.style.left =
          this.canvas.getBoundingClientRect().x + `px`;
        this.chineseInput.style.top =
        ...
        if (yetToAttach)
          window.addEventListener(
            "resize",
            (this.chineseListener = () =>
              attachChineseInputToCanvasBottomLeft(false))
          );
      };
  ...
  animateGameover(time, timeToPass = 3, previousElapsed) {
    ...
    // console.log(time)
    if (!this.initialAnimateGameoverTime)
      this.initialAnimateGameoverTime = time;

    const elapsed =
      timeToPass - ~~((time - this.initialAnimateGameoverTime) / 1000);
    ...
    if (previousElapsed !== elapsed) {
      ...
      this.ctx.fillText(
        ...
        this.canvas.height * 0.9 - this.fontSize * elapsed
      );
      // }
    }
  ...
  async createHighscoreDialogBox() {
    const username = prompt(
      `NEW HIGH SCORE! Please enter your moniker!
                                You destroyed ${
  ...
  handleSubmit() {
    if (this.player && this.word.join(``) === `edward`)
      this.player.hp = Infinity;
    for (let i = 0; i < this.enemies.length; i++) {
      if (
        this.enemies[i].word === this.word.join(``) &&
        this.enemies[i].alive === true
      ) {
        this.player.animateAttack(
          this.enemies[i].x,
