...
class Crono extends Actor {
  ...
  checkCollision() {
    // can use leetcodes overlapping rectangles to detect. want to add a get method for this.x,this.y and this.x + canvaswidthtakenupbycrono, etc.
    for (const enemy of this.enemies) {
      if (enemy.alive === false) continue;

      if (((enemy.x - this.x) ** 2 + (enemy.y - this.y) ** 2) ** 0.5 < 20) {
        this.takeDamage();
  ...
  animateAttack(x, y, attackPicture) {
    ...
    this.angleStartClockwise =
      -Math.atan(
        (leftsY - this.circleCenterY) / (this.circleCenterX - leftsX)
      ) + Math.PI;
